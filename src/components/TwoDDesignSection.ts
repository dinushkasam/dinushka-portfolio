...
import { type TwoDProject } from '../types/index';
  ...
  return `
    <a href="2d-project.html?id=${project.id}" class="project-card group block">
      <div class="h-56 sm:h-64 md:h-72 bg-gradient-to-br from-gray-900 to-black flex items-center justify-center border-b border-gray-700 group-hover:border-gray-600 transition-colors overflow-hidden rounded-t-xl">
        <img src="${project.thumbnail}" alt="${project.name}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.style.display='none'; this.parentElement.innerHTML='<span class=\\'text-gray-600 text-sm\\'>Image coming soon</span>';">
      </div>
      <div class="p-4 sm:p-6 bg-gray-800/30 rounded-b-xl">
        <h3 class="text-lg sm:text-xl font-bold text-white text-center">${project.name}</h3>
      </div>
    </a>
  `;
};

export const TwoDDesignSection = (projects: TwoDProject[]): string => {
  const projectCards = projects.map(renderTwoDCard).join('');
